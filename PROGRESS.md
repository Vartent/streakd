...
- **Status**:
  ...
  - Streak test tooling ADDED AND VERIFIED LIVE (2nd pass same day): streakd got ReplaceHistory/ShiftHistory/SetFreezes engine ops (@ d1108b8, tested incl. PK-safe mark shifting via delete+reinsert CTE); kanji exposes them as /api/streak/test/{setup,shift,freezes,reset} gated on STREAK_TEST_ENABLED=1 (dev compose sets it; prod 404s). Principle: manipulate history, never the clock. Full battery verified via curl: setup 20d -> alive/20/2 freezes; shift 1 -> rollover (unearned, count kept); shift 1 -> frozen; train -> 21 + freeze spend + re-earn; setup ended-4d-ago -> broken 0; reset -> 0. Test account left at 0.
  - PROD DEPLOYED (3rd pass same day): streak commit 490e611 pushed to kanji master (feedback WIP wiring stripped from committed main.go only — working tree keeps it; prod already runs feedback via tar deploys). Deploy = rsync backend/ overlay onto /home/admin/kanji_cards (server is NOT a git repo; tar/rsync convention) + docker-compose v1 build + fresh create. Backups taken first: DB dump backups/production/kanji_cards_pre_streakd_20260702_143206.dump + backend.bak.pre_streakd_20260702_143206. VERIFIED: 5 real streaks seeded+recounted, lapsed users read 0 with longest preserved, today's trainee alive at 1, kanji data intact (2216), health OK, /api/streak/test/* returns 404 on prod. ~4.5 min backend downtime (docker-compose 1.29 ContainerConfig recreate bug; fix: rm renamed leftover container, then up -d --no-deps creates fresh).
- **Next steps**:
